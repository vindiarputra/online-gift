...
export async function POST(request: Request) {
    const {id, label, price, quantity, description, isNew, onSale, isFeatured, image} = await request.json();
    const {data, error} = await supabase.from("cart").insert([{label, price, quantity, description, isNew, onSale, isFeatured, image, productId: id}]);
    if (error) {
